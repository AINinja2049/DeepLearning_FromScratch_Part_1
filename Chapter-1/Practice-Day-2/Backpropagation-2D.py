...
def backprop(X: ndarray,
    ...
    dSdN = deriv(sigma,N)
    dLdN = dLdS*dSdN
    dNdX = np.transpose(W,(1,0)) # N = (x1*w1+x2*w2+x3*w3) = N'(X) = [w1 w2 w3] = W^T   derivatives shapes are controlled by wrt X 
    dNdW = np.transpose(X,(1,0)) # N = (x1*w1+x2*w2+x3*w3) = N'(W) = [x1 x2 x3]^T = X^T derivatives shapes are controlled by wrt W
